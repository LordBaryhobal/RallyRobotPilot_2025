...
def main():
    ...
    index:int = 0
    complete_trajectory:list[TrajectoryPoint] = []
    segment_length:int = 100
    segments_length:int = to.count_segment(segment_length)
    while index < segments_length: 
        segment: TrajectorySegment = to.load_segment(index,segment_length)
        ...
        complete_trajectory.extend(traj_sim)
        TrajectoryPoint.save_list_to_json(complete_trajectory,trackname,index)
        index += 1
