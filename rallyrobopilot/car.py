...
class Car(Entity):
    ...
    def hand_brake(self):
        ...
        if held_keys["space"]:
            if self.rotation_speed < 0:
                self.rotation_speed -= 3 * DELTA_TIME
    ...
    def compute_steering(self):
        # Steering
        self.rotation_y += self.rotation_speed * 50 * DELTA_TIME

        # The car's linear momentum decreases the rotation.
        if self.rotation_speed > 0:
            self.rotation_speed -= self.speed / 6 * DELTA_TIME
        elif self.rotation_speed < 0:
            self.rotation_speed += self.speed / 6 * DELTA_TIME
        ...
        if self.speed > 0.5 or self.speed < -0.5:
            ...
            # If no keys pressed, the rotation speed goes down.
                ...
                if self.rotation_speed > 0:
                    self.rotation_speed -= 5 * DELTA_TIME
                elif self.rotation_speed < 0:
                    self.rotation_speed += 5 * DELTA_TIME
        else:
            self.rotation_speed = 0
    ...
    def update(self):
        ...
        if self.keys[self.controls[1]] or held_keys[self.controls[1]] or held_keys["left arrow"] or self.keys[self.controls[3]] or held_keys[self.controls[3]] or held_keys["right arrow"]:
            ...
            travelled_dist = abs(self.speed * DELTA_TIME)
        ...
        total_dist_to_move = self.speed * DELTA_TIME
